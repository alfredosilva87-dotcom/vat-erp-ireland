...
import { buildAccountingPdf } from "@/lib/accounting/exportDocs";
...
/**
 * DRE, Balanco e Balancete num PDF so, no papel timbrado do escritorio.
 *
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const nome = `${c.atual.client?.client_code || "cliente"}-contas-${ano}-${visao}.pdf`
    .replace(/[^\w.\-]/g, "_");
  ...
  return new NextResponse(new Blob([await buildAccountingPdf(c)]), {
    headers: {
