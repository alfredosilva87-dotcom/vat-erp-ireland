...
import { buildLedgerWorkbook } from "@/lib/accounting/exportLedger";
...
/**
 * O razao em Excel: uma aba agrupada por conta e outra plana, para filtrar.
 *
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const nome = `${razao.client?.client_code || "cliente"}-razao-${de}-a-${ate}.xlsx`
    .replace(/[^\w.\-]/g, "_");
  ...
  return new NextResponse(new Blob([await buildLedgerWorkbook(razao, escritorio)]), {
    headers: {
