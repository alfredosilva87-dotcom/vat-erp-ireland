...
export default function ClientMail({ params }: { params: { id: string } }) {
  ...
  const load = useCallback(async () => {
    const [r, s] = await Promise.all([
      fetch(`/api/clients/${params.id}/mail-routes`, { cache: "no-store" }),
      fetch(`/api/clients/${params.id}/mail-senders`, { cache: "no-store" }),
    ]);
  ...
  async function createRoute(direction: "purchase" | "sale") {
    setBusy(true);
    try {
      const res = await fetch(`/api/clients/${params.id}/mail-routes`, {
  ...
  async function patchRoute(id: string, body: any, message: string) {
    setBusy(true);
    try {
      const res = await fetch(`/api/clients/${params.id}/mail-routes/${id}`, {
  ...
  async function addSender() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/clients/${params.id}/mail-senders`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  async function removeSender(id: string) {
    setBusy(true);
    try {
      await fetch(`/api/clients/${params.id}/mail-senders/${id}`, { method: "DELETE" });
      await load();
    } finally { setBusy(false); }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rise">
        <h1 className="font-display text-xl font-semibold tracking-tight">Entrada por e-mail</h1>
      ...
      {!configured && (
        <div className="card rise border border-warning/30 bg-warning-50 p-4 text-sm text-warning">
